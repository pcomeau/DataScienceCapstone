...
* Flattening of Catagorical Variables 
...
It is important to note that this report and its associated artifacts are completely re-producible. Random number generation seeds are set before all pertinent operations to ensure repeatability. Complete R markdown source code can be found at: TODO: PUT THE GITHUB LINK here 
...
nr <- nrow(final.ds)
```
In addition to the continuous variable of interest "review_count", the cleaned and prepared data set contains a total of 52 variables, three continuous variables and 49 categorical variables. The final data set containes `r nr` rows of data.
...
names(final.ds)
```
Given the few continuous variables, "Star Rating" and "latitude/longitude", it appears that restaurants  having higher review counts generally fall in the range of 4.0 to 4.5 stars as can be seen from the following side by side boxplot.    
...
```
Next a Random Forest model using the caret package train function using the features deemed relivent by the Boruta feature selection algorithm is fit. 
...
* for each additonal "`r mi$coeff[3]`" (rating), restaruants receive `r abs(round(mi$low[3],2))` `r mi$low.dir[3]` to `r abs(round(mi$high[3],2))` `r mi$high.dir[3]` reviews
